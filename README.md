...
| `inspect ip` | Show your public IP, local interface addresses, or both |
...
swk inspect ip --local --all            # includes them, no network lookup
swk inspect ip --local --json

# Everything: public plus every local address
swk inspect ip --all
