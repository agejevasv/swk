...
func init() {
	datetimeCmd.Flags().StringVarP(&dtFrom, "from", "f", "auto", "input format (unix, unixms, iso, rfc2822, human, auto)")
	datetimeCmd.Flags().StringVarP(&dtTo, "to", "t", "iso", "output format (unix, unixms, iso, rfc2822, human)")
	datetimeCmd.Flags().StringVar(&dtTz, "tz", "Local", "target timezone (e.g. UTC, America/New_York)")
	Cmd.AddCommand(datetimeCmd)
}
