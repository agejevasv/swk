...
func init() {
	numbaseCmd.Flags().StringVarP(&nbFrom, "from", "f", "dec", "source base (bin, oct, dec, hex)")
	numbaseCmd.Flags().StringVarP(&nbTo, "to", "t", "hex", "target base (bin, oct, dec, hex)")
	Cmd.AddCommand(numbaseCmd)
}
